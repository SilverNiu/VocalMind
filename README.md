...
- 语音：`vocalmind.audio.Emotion2VecAudioRecognizer` 使用 FunASR `AutoModel(model="iic/emotion2vec_plus_large", hub="ms")`，模型缓存由 FunASR/ModelScope 管理，不提交到仓库。普通 `pytest` 只测解析和 API mock，不会下载大模型。
- 人脸：`vocalmind.face.EmotiEffFaceRecognizer` 默认走 EmotiEffLib ONNX + `mbf_va_mtl`，新增人脸检测/裁剪步骤。若图片没有检测到人脸，API 返回明确 JSON 错误。
...
如果报 `model_unavailable` 或缺依赖，先按上面的 `requirements-face.txt` / `requirements-audio.txt` 安装。语音大模型首次运行会从 ModelScope 下载 `iic/emotion2vec_plus_large`，网络慢时可改用团队共享的本地缓存。
