...
import { ArrowLeft, Activity, Clock, MessageCircle, Info, Mic, ScanFace, Check, X, Edit3 } from 'lucide-react';
...
export function ReportView({ record, onBack, onFeedbackSubmit }: ReportViewProps) {
  ...
  const conversationTranscript = record.transcript?.filter(line =>
    (line.role === 'assistant' || line.role === 'user') &&
    line.text.trim() &&
    !line.text.includes('本轮未返回可显示文本')
  ) || [];
  const emotionVolatility = record.report.emotion_trend === 'changed' ? '有变化' : '稳定';
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-6 mb-6">
            ...
            <div className="flex gap-8 text-center md:text-right">
              ...
              <div>
                ...
                <p className="text-[16px] font-medium text-slate-700">{emotionVolatility}</p>
              </div>
            </div>
          </div>
      ...
      {conversationTranscript.length > 0 && (
        ...
        >
          <GlassCard className="p-5 bg-white/60">
            <h4 className="text-[14px] font-medium text-slate-700 mb-3 flex items-center gap-2">
              <MessageCircle className="w-4 h-4 text-blue-500" /> 对话记录
            </h4>
            <div className="max-h-[180px] space-y-2 overflow-y-auto pr-1">
              {conversationTranscript.slice(-8).map(line => (
                <div
                  key={line.id}
                  className={
                    line.role === 'user'
                      ? 'ml-auto max-w-[86%] rounded-lg bg-blue-500 px-3 py-2 text-[13px] leading-relaxed text-white'
                      : 'max-w-[86%] rounded-lg border border-blue-100/70 bg-white/78 px-3 py-2 text-[13px] leading-relaxed text-slate-600'
                  }
                >
                  <div className={line.role === 'user' ? 'mb-1 text-[11px] text-blue-100' : 'mb-1 text-[11px] text-blue-500'}>
                    {line.role === 'user' ? '我' : 'MiniCPM'}
                  </div>
                  <div>{line.text}</div>
                </div>
              ))}
